...
| path         | Persistence path for QdrantLocal. Eg. `everi_ai_qgpt_vectordb_qdrant/local_vectordb/qdrant`|
...
```yaml
qdrant:
  path: everi_ai_qgpt_vectordb_qdrant/local_vectordb/qdrant
```
...
| Field        | Description |
|--------------|-------------|
| uri     | Default is set to "everi_ai_qgpt_vectordb_qdrant/local_vectordb/milvus/milvus_local.db" as a local file; you can also set up a more performant Milvus server on docker or k8s e.g.http://localhost:19530, as your uri; To use Zilliz Cloud, adjust the uri and token to Endpoint and Api key in Zilliz Cloud.|
...
To obtain a local setup (disk-based database) without running a Milvus server, configure the uri value in settings.yaml, to store in everi_ai_qgpt_vectordb_qdrant/local_vectordb/milvus/milvus_local.db.
...
By default `chroma` will use a disk-based database stored in everi_ai_qgpt_vectordb_qdrant_path / "chroma_db" (being everi_ai_qgpt_vectordb_qdrant_path defined in settings.yaml)
...
| **schema_name** | The database schema to use. Default is `local_vectordb`       |
...
postgres:
  ...
  user: postgres
  password: <PASSWORD>
  schema_name: local_vectordb
```

The following table will be created in the database
```
postgres=# \d local_vectordb.data_embeddings
                                      Table "local_vectordb.data_embeddings"
  Column   |       Type        | Collation | Nullable |                         Default
-----------+-------------------+-----------+----------+---------------------------------------------------------
 id        | bigint            |           | not null | nextval('local_vectordb.data_embeddings_id_seq'::regclass)
