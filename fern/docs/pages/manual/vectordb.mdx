...
| path         | Persistence path for QdrantLocal. Eg. `qgpt_vectordb_qdrant/local_vectordb/qdrant`|
...
```yaml
qdrant:
  path: qgpt_vectordb_qdrant/local_vectordb/qdrant
```
...
| Field        | Description |
|--------------|-------------|
| uri     | Default is set to "qgpt_vectordb_qdrant/local_vectordb/milvus/milvus_local.db" as a local file; you can also set up a more performant Milvus server on docker or k8s e.g.http://localhost:19530, as your uri; To use Zilliz Cloud, adjust the uri and token to Endpoint and Api key in Zilliz Cloud.|
...
To obtain a local setup (disk-based database) without running a Milvus server, configure the uri value in settings.yaml, to store in qgpt_vectordb_qdrant/local_vectordb/milvus/milvus_local.db.
...
By default `chroma` will use a disk-based database stored in qgpt_vectordb_qdrant_path / "chroma_db" (being qgpt_vectordb_qdrant_path defined in settings.yaml)
