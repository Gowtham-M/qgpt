...
from everi_ai_qgpt_core.open_ai.openai_models import (
    OpenAICompletion,
    OpenAIMessage,
    to_openai_response,
    to_openai_sse_stream,
)
from everi_ai_qgpt_core.server.chat.chat_service import ChatService
...
@chat_router.post("/chat/autocomplete")
def autocomplete(request: AutocompleteRequest):
    """ Predicts user input dynamically """
    prompt = f"Continue this senctence : {request.partial_text}"

    try:
        response = ollama.chat(model="mistral",message=[{"role":"user","content":prompt}])
